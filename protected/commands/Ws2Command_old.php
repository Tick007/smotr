...
class WS2Command extends CConsoleCommand
{
    ...
    private $cyclogramShowData = false;
    ...
    public function initializeData()
    {
        ...
        $sql = "SELECT `id`, `LocalNum`, `NameEq`, `Description` FROM `j400_equipment`";
    ...
    public function onReceivecallback(ConnectionInterface $from, $msg, \SplObjectStorage $clients)
    {
       // echo 'onReceive: ' . $msg . "\r\n";
        ...
        //$this->sendConf($msg, $client);
        
       // $this->clients->current()->send("Client $from->resourceId said $msg");
    ...
    public function send100()
    {
        if (isset($this->clients)) {
            ...
            if($this->cyclogramShowData==true && $this->cyclogramData!=null){
            ...
            foreach ($this->clients as $client) {
                ...
                else $client->send(json_encode($data, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE | JSON_NUMERIC_CHECK | JSON_PRETTY_PRINT));
            }
            // print_r($data);
        }
    }
 ...
 */
    ...
    private function processCyclogramCommand($command){
        $this->cyclogramData = null;
        //print_r($command);
        if($command->dialog=='open'){ ///////Значит нужно сформировать в данных для интерфейса секцию с циклограммами
            $this->cyclogramShowData=true;
        }
        if($command->dialog=='close'){
            $this->cyclogramShowData=false;
        }
        
        //echo 'all_list:'."\n\r";
        //print_r( $this->db_table_list);
        //echo "\n\r";
        ...
        $sql = 'select 	
                ...
                CKG_Description,
                CKG_ID,
                READONLY from j400_ciclogramms';
        if ($this->pdo_conn instanceof PDO) {
            try {
                $rez = $this->pdo_conn->query($sql);
            } catch (PDOException $e) {
            ...
            if (isset($rez) && $rez != null && $rez->rowCount() > 0) {
                foreach ($rez as $row) {
                    if(in_array(strtolower($row['CKG_Name']), $this->db_table_list)){
                        $this->getCyclogram($row['CKG_Name']);
