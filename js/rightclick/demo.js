$(document).ready(function(){
	...
	context.attach('#connstatus_4000', [
		
		{header: 'CONF'},
			{text: 'Запросить конфигурацию', href: '#', target:'_blank', action: function(e){
				e.preventDefault();
				make_request();
			}},
			{text: 'Порт', subMenu: [
			{text: 'Закрыть', href: '#', target:'_blank', action: function(e){
				e.preventDefault();
				armsocketcontrol(4000, 'close');
			}},
			{text: 'Открыть', href: '#', target:'_blank', action: function(e){
				e.preventDefault();
				armsocketcontrol(4000, 'open');
			}}
	...
	context.attach('#connstatus_3033', [
		
		{header: 'RNG'},
			{text: 'Измерение дальности', href: '#', target:'_blank', action: function(e){
				e.preventDefault();
				$('#rngcall').click();
			}},
			{text: 'Порт', subMenu: [
			{text: 'Закрыть', href: '#', target:'_blank', action: function(e){
				e.preventDefault();
				armsocketcontrol(3033, 'close');
			}},
			{text: 'Открыть', href: '#', target:'_blank', action: function(e){
				e.preventDefault();
				armsocketcontrol(3033, 'open');
			}}
		...
		context.attach('#connstatus_3031', [
		
		{header: 'ANGL'},
			{text: 'Измерение углов', href: '#', target:'_blank', action: function(e){
				e.preventDefault();
				$('#anglcall').click();
			}},
			{text: 'Порт', subMenu: [
			{text: 'Закрыть', href: '#', target:'_blank', action: function(e){
				e.preventDefault();
				armsocketcontrol(3031, 'close');
			}},
			{text: 'Открыть', href: '#', target:'_blank', action: function(e){
				e.preventDefault();
				armsocketcontrol(3031, 'open');
			}}
		...
		context.attach('#connstatus_3010', [
		                            		
		{header: 'TM'},
			{text: 'Телеметрия', href: '#', target:'_blank', action: function(e){
				e.preventDefault();
				$('#tmcall').click();
			}},
			{text: 'Порт', subMenu: [
			{text: 'Закрыть', href: '#', target:'_blank', action: function(e){
				e.preventDefault();
				armsocketcontrol(3010, 'close');
			}},
			{text: 'Открыть', href: '#', target:'_blank', action: function(e){
				e.preventDefault();
				armsocketcontrol(3010, 'open');
			}}
